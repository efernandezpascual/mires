...
read.table("data/dataloggers/Loggers.txt", sep = "\t", header = TRUE) %>%
...
wd <- getwd()
setwd(paste(wd, "/data/dataloggers", sep = ""))
...
logs.clean %>% 
  filter(! Site == "Los Cándanos") -> logs.clean2
...
write.csv(temperatures, "results/clean_data/temperatures.csv", row.names = FALSE)
write.csv(header, "results/clean_data/header.csv", row.names = FALSE)
save(temperatures, header, file = "results/clean_data/temperatures.RData")
